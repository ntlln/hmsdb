...
public class adminpharmacyFrame extends javax.swing.JFrame {

    public adminpharmacyFrame() {
        initComponents();
        retrieveData();
    }
    ...
    private void toBillingActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_toBillingActionPerformed
        String patientNameOrId = JOptionPane.showInputDialog(null, "Enter the patient name or ID:");
        if (patientNameOrId != null && !patientNameOrId.isEmpty()) {
            try {
                Connection connection = connect.getConnection();
                String query = "SELECT PID, `Medication Total` FROM billingTable WHERE PID IN (SELECT PID FROM patientTable WHERE `Patient Name` = ? OR PID = ?)";
                PreparedStatement preparedStatement = connection.prepareStatement(query);
    ...
    public static void main(String args[]) {
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new adminpharmacyFrame().setVisible(true);
            }
        });
    }
